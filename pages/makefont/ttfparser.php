...
class TTFParser
{
	...
	function ParseOffsetTable()
	{
		...
		$numgestion = $this->ReadUShort();
		$this->Skip(3*2); // searchRange, entrySelector, rangeShift
		$this->gestion = array();
		for($i=0;$i<$numgestion;$i++)
		{
			$tag = $this->Read(4);
			$checkSum = $this->Read(4);
			$offset = $this->ReadULong();
			$length = $this->ReadULong();
			$this->gestion[$tag] = array('offset'=>$offset, 'length'=>$length, 'checkSum'=>$checkSum);
		}
	}	
	...
	function ParseGlyf()
	{
		$tableOffset = $this->gestion['glyf']['offset'];
	...
	function ParseCmap()
	{
		...
		$idRangeOffset = array();
		$this->chars = array();
		fseek($this->f, $this->gestion['cmap']['offset']+$offset31, SEEK_SET);
	...
	{
		$this->Seek('name');
		$tableOffset = $this->gestion['name']['offset'];
		$this->postScriptName = '';
		$this->Skip(2); // format
	...
	function BuildCmap()
	{
		...
		$data = pack('nn', 0, 1); // version, numgestion
	...
	function BuildHhea()
	{
		...
		$data = substr_replace($this->gestion['hhea']['data'], pack('n',$numberOfHMetrics), 4+15*2, 2);
	...
	function BuildMaxp()
	{
		...
		$data = substr_replace($this->gestion['maxp']['data'], pack('n',$numGlyphs), 4, 2);
	...
	function BuildFont()
	{
		...
		{
			if(isset($this->gestion[$tag]))
				$tags[] = $tag;
		}
		$numgestion = count($tags);
		$offset = 12 + 16*$numgestion;
		foreach($tags as $tag)
		{
			if(!isset($this->gestion[$tag]['data']))
				$this->LoadTable($tag);
			$this->gestion[$tag]['offset'] = $offset;
			$offset += strlen($this->gestion[$tag]['data']);
		}
		...
		$rangeShift = 16*$numgestion - $searchRange;
		$offsetTable = pack('nnnnnn', 1, 0, $numgestion, $searchRange, $entrySelector, $rangeShift);
		foreach($tags as $tag)
		{
			$table = $this->gestion[$tag];
		...
		foreach($tags as $tag)
			$s .= $this->gestion[$tag]['checkSum'];
		$a = unpack('n2', $this->CheckSum($s));
		$high = 0xB1B0 + ($a[1]^0xFFFF);
		$low = 0xAFBA + ($a[2]^0xFFFF) + 1;
		$checkSumAdjustment = pack('nn', $high+($low>>16), $low);
		$this->gestion['head']['data'] = substr_replace($this->gestion['head']['data'], $checkSumAdjustment, 8, 4);

		$font = $offsetTable;
		foreach($tags as $tag)
			$font .= $this->gestion[$tag]['data'];
	...
	{
		$this->Seek($tag);
		$length = $this->gestion[$tag]['length'];
		$n = $length % 4;
		if($n>0)
			$length += 4 - $n;
		$this->gestion[$tag]['data'] = $this->Read($length);
	}
	...
	function SetTable($tag, $data)
	{
		...
		$this->gestion[$tag]['data'] = $data;
		$this->gestion[$tag]['length'] = $length;
		$this->gestion[$tag]['checkSum'] = $this->CheckSum($data);
	}
	...
	function Seek($tag)
	{
		...
		fseek($this->f, $this->gestion[$tag]['offset'], SEEK_SET);
	}
